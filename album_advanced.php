...
<html>
	...
	<body>
		...
		?>
		<hr>
        <p>Recherche d'un album par recherche avancée : </p>
        <form method="POST" action="artiste_advanced.php">
            <label>Nom</label> : <input type="text" name="nom"/>
            <label>Année de sortie : </label> : <input type="text" name="date" />
        ...
        <?php
            ...
            // $genre = pg_escape_string($genre);
             //$groupe = pg_escape_string($groupe);
              ...
              $raw_results = pg_query($d, "SELECT * FROM album, artiste, artalbum
              WHERE nart = nom_a AND (nom LIKE '%".$nom."%') AND (YEAR(date_sortie) LIKE '%".$date."%') AND (genre_pred LIKE '%".$genre."%')") or die(pg_last_error());
            ...
            // $results = mysql_fetch_array($raw_results) puts data from database into array, while it's valid it does the loop
                        ...
                        echo '<section>';
                        echo "<h2> Voici : </h2>";
                        echo "<p> Nom : ".$results['nom_a']."</p>";
                        echo "<p> Durée de l'album  : ".$results['tot_lenght']."</p>";
                        echo "<p> Date de sortie : ".$results['date_sortie']."</p>";
                        echo "<p> Genre : ".$results['genre_pred']."</p>";
