	...
	<body>
		...
		<hr>
         <?php
            ...
            $query = 'SELECT nom, lieu, date_fd, date_ff FROM festival ORDER BY date_fd DESC';
            $data = pg_query($d, $query) or die(pg_last_error());
            if(pg_num_rows($data) > 0){
                 while($results = pg_fetch_array($data)){
                     echo "<p> Festival :  ".$results['nom']." commençant le ".$results['date_fd']." et se terminant le  ".$results['date_ff']." à l'adresse :
                     ".$results['lieu']. "</p>";
                 }
            }
        ?>
